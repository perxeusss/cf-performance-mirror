(async function () {
  ...
  function updateTheme() {
    ...
    if (newIsDark !== previousTheme.isDark ||
      ...
      if (currentCategory && rawSubmissions.length > 0) {
        const timelineValue = timelineSelect.value === "custom" 
          ? { type: "custom", start: startDateInput.value, end: endDateInput.value }
          : timelineSelect.value;
        const modeData = recalcForMode(modeSelect.value, timelineValue, currentCategory);
        renderTableForCategory(modeData, currentCategory);
        renderWeakTopics(weakContest, `High WA% (${currentCategory})`, modeData.contestFriction);
  ...
  function recalcForMode(mode, timelineMonths, categoryFilter) {
    ...
    filteredSubmissions.forEach(s => {
      ...
      tags.forEach(t => {
        categoryTopicSolved[cat][t] = (categoryTopicSolved[cat][t] || 0) + 1;
      });
  ...
  function renderCategory(cat) {
    ...
    renderWeakTopics(weakContest, `High WA% (${cat})`, modeData.contestFriction);
    renderWeakTopics(weakGlobal, "High WA% (Overall)", modeData.globalFriction);
  }
